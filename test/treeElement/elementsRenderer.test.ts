import type { GetTree, IsNodeSelected } from "treeElement/methodTypes";
import type { NodeData } from "treeElement/node";
import type { IconElement, OnCreateLi } from "treeElement/options";
...
describe("renderFromRoot", () => {
    ...
    it("attaches the element to the node", () => {
        ...
        expect(node.element).toBe(getTreeListElement(getTreeItem("node1")));
        expect(setNodeElement).toHaveBeenCalledTimes(6);
        expect(setNodeElement).toHaveBeenCalledWith(node.element, node);
    });
    ...
    it("calls onCreateLi for every node", () => {
        ...
        expect(onCreateLi).toHaveBeenCalledWith(node, node.element, false);
    });
    ...
    it("adds the rtl class to the root list when rtl is true", () => {
        ...
        expect(screen.getByRole("tree")).toHaveClass("tree-element-rtl");
        expect(screen.getAllByRole("group")[0]).not.toHaveClass("tree-element-rtl");
    });
    ...
    it("doesn't add the rtl class to the root list when rtl is undefined", () => {
        ...
        expect(screen.getByRole("tree")).not.toHaveClass("tree-element-rtl");
    });
    ...
    it("adds the dnd class to the lists when dragAndDrop is true", () => {
        ...
        expect(screen.getByRole("tree")).toHaveClass("tree-element-dnd");
        expect(screen.getAllByRole("group")[0]).toHaveClass("tree-element-dnd");
    });
    ...
    it("doesn't add the dnd class to the lists when dragAndDrop is false", () => {
        ...
        expect(screen.getByRole("tree")).not.toHaveClass("tree-element-dnd");
        expect(screen.getAllByRole("group")[0]).not.toHaveClass("tree-element-dnd");
    });
    ...
    it("renders an open folder", () => {
        ...
        expect(getTreeListElement(treeItem)).not.toHaveClass("tree-element-closed");
        expect(getTreeButton(treeItem)).not.toHaveClass("tree-element-closed");
    ...
    it("renders a selected folder", () => {
        ...
        expect(getTreeListElement(treeItem)).toHaveClass("tree-element-selected");
    ...
    it("renders the button on the right when buttonLeft is false", () => {
        ...
        expect(treeItem).toHaveClass("tree-element-title-button-right");
        expect(getTreeButton(treeItem)).toHaveClass("tree-element-toggler-right");
        // eslint-disable-next-line testing-library/no-node-access
        expect(treeItem.nextSibling).toHaveClass("tree-element-toggler");
    });
    ...
    it("renders the button on the left when buttonLeft is true", () => {
        ...
        expect(treeItem).toHaveClass("tree-element-title-button-left");
        expect(getTreeButton(treeItem)).toHaveClass("tree-element-toggler-left");
        // eslint-disable-next-line testing-library/no-node-access
        expect(treeItem.previousSibling).toHaveClass("tree-element-toggler");
    });
    ...
    it("renders no icon when the icon is not an element", () => {
        const { renderer } = createRenderer({
            closedIcon: {} as unknown as IconElement,
        });

        renderer.renderFromRoot();

        expect(getTreeButton(getTreeItem("node1"))).toBeEmptyDOMElement();
    });
